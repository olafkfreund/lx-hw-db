                    ...
                    if let DetectionData::Dmidecode(data) = detection_result.data {
                        ...
                        if detection_result.success {
                            ...
                            if let Some(system) = &data.system {
                                assert!(
                                    !system.manufacturer.is_empty(),
                                    "System manufacturer should not be empty"
                                );
                                assert!(
                                    !system.product_name.is_empty(),
                            ...
                            if let Some(bios) = &data.bios {
                                ...
                                assert!(
                                    !bios.version.is_empty(),
                                    "BIOS version should not be empty"
                                );
                            }
                        } else {
                            // On failure, we should have meaningful error messages
                            assert!(
                                !detection_result.errors.is_empty(),
                                "Failed detection should have error messages"
                            );
                        }
