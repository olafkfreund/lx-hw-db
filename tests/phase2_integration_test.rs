...
async fn test_phase2_hardware_detection_framework() {
    ...
    match results {
        Ok(detection_results) => {
                ...
                match &result.data {
                    DetectionData::Lshw(lshw_data) => {
                        ...
                        if let Some(summary) = &lshw_data.summary {
                            println!(
                                "lshw summary: {} components across {} classes",
                    ...
                    DetectionData::Dmidecode(dmidecode_data) => {
                        ...
                        if let Some(summary) = &dmidecode_data.summary {
                            println!(
                                "dmidecode summary: {} MB total memory, {} slots used",
                                summary.total_memory_mb, summary.memory_slots_used
                            );
            ...
            let successful_detections = detection_results.iter().filter(|r| r.success).count();

            if successful_detections > 0 {
                println!("✓ At least one detector provided successful results");
            } else {
                println!(
                    "⚠ No successful detections (may be due to privileges or system configuration)"
                );
            }
            ...
            if sensitive_data_found {
                println!(
                    "✓ Privacy-sensitive data detected across detectors (ready for anonymization)"
                );
            } else {
            ...
            let total_errors: usize = detection_results.iter().map(|r| r.errors.len()).sum();
