...
async fn test_tool_filtering() -> Result<()> {
    ...
    for detector in &available {
        ...
        assert!(name == "lshw" || name == "lspci", "Unexpected detector found: {}", name);
    }
...
async fn test_invalid_tool_filtering() {
    ...
    assert!(
        error_msg.contains("Unknown detection tools"),
        "Error message should mention unknown tools"
    );
    assert!(error_msg.contains("Available tools"), "Error message should list available tools");
}
