...
impl<'a> CompatibilityAnalyzer<'a> {
    ...
    pub fn analyze(&self) -> Result<CompatibilityAnalysis> {
        log::info!("Starting comprehensive compatibility analysis of {} reports", self.reports.len());
    ...
    fn analyze_health_metrics(&self) -> Result<HealthMetrics> {
        ...
        let overall_score = self.calculate_overall_health_score(&data_quality, &coverage, &freshness);
    ...
    fn assess_data_quality(&self) -> Result<DataQuality> {
        ...
        let complete_reports = self.reports.iter()
            .filter(|r| self.is_report_complete(r))
            .count() as f64;
    ...
    fn assess_data_freshness(&self) -> Result<FreshnessMetrics> {
        ...
        let freshness_score = self.calculate_freshness_score(average_age_days, recent_reports_percentage);
    ...
    fn detect_regressions(&self) -> Result<Vec<CompatibilityRegression>> {
        ...
        for (hardware_id, kernel_scores) in hardware_kernel_scores {
            if let Some(regression) = self.detect_hardware_regression(&hardware_id, &kernel_scores)? {
    ...
    fn calculate_overall_health_score(&self, quality: &DataQuality, coverage: &CoverageAnalysis, freshness: &FreshnessMetrics) -> u8 {
        let weighted_score = (
            quality.completeness_score as f64 * 0.3 +
            quality.consistency_score as f64 * 0.2 +
            coverage.vendor_coverage as f64 * 0.2 +
            coverage.category_coverage as f64 * 0.15 +
            freshness.freshness_score as f64 * 0.15
        ) as u8;
    ...
    fn calculate_submission_velocity(&self) -> f64 {
        ...
        let now = Utc::now();
        let oldest = self.reports.iter()
            .map(|r| r.metadata.submission_date)
            .min()
            .unwrap_or(now);
    ...
    fn calculate_freshness_score(&self, avg_age: f64, recent_percentage: f64) -> u8 {
        ...
        let age_score = if avg_age <= 30.0 { 100.0 } else { (100.0 - (avg_age - 30.0) * 2.0).max(0.0) };
        let recent_score = recent_percentage;
        
        ((age_score * 0.6 + recent_score * 0.4) as u8).min(100)
    }
    
    fn calculate_kernel_trend(&self, kernel: &str, data: &[(DateTime<Utc>, f64)]) -> Result<KernelTrend> {
        // Calculate trend statistics for a kernel version
        let avg_compatibility = data.iter().map(|(_, score)| *score).sum::<f64>() / data.len() as f64;
    ...
    fn analyze_single_vendor(&self, vendor: &str, reports: &[&IndexedReport]) -> Result<VendorAnalysis> {
        let avg_score = reports.iter()
            .map(|r| r.compatibility.status.to_score() as f64)
            .sum::<f64>() / reports.len() as f64;
        
        let overall_trend = if avg_score >= 80.0 {
            TrendDirection::Improving
        } else {
            TrendDirection::Stable
        };
    ...
    fn analyze_single_category(&self, category: &str, reports: &[&IndexedReport]) -> Result<CategoryInsight> {
        let compatibility_score = reports.iter()
            .map(|r| r.compatibility.status.to_score() as f64)
            .sum::<f64>() / reports.len() as f64;
    ...
    fn analyze_single_distribution(&self, distribution: &str, reports: &[&IndexedReport]) -> Result<DistributionAnalysis> {
        let compatibility_score = reports.iter()
            .map(|r| r.compatibility.status.to_score() as f64)
            .sum::<f64>() / reports.len() as f64;
    ...
    fn generate_category_recommendations(&self) -> Result<HashMap<String, Vec<HardwareRecommendation>>> {
