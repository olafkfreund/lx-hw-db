...
fn validate_hardware_counts(
    ...
    let device_checks = [
        (
            ...
            "No graphics devices detected. This is unusual for most systems",
        ),
        (
            report.network.is_empty(),
            "No network devices detected. This is unusual for most systems",
        ),
        (report.storage.is_empty(), "No storage devices detected. This is highly unusual"),
    ];

    warnings.extend(device_checks.iter().filter_map(|(is_empty, message)| {
        if *is_empty {
...
fn validate_hardware_compatibility(
    ...
    for storage in &report.storage {
        ...
        match (storage.device_type.as_str(), storage.interface.as_deref()) {
            ...
            ("SSD", Some("IDE")) => {
                warnings
                    .push(format!("SSD '{}' with IDE interface is very unusual", storage.model));
            }
            _ => {}
        }
    }
...
fn validate_system_coherence(
    ...
    if kernel_parts.len() < 2 {
        return Err(ValidationError::BusinessLogicError {
            ...
            message: format!("Invalid kernel version format: {}", report.system.kernel_version),
        });
    }
    ...
    if let Ok(major) = kernel_parts[0].parse::<u32>() {
        if !(2..=10).contains(&major) {
            warnings.push(format!("Unusual kernel major version: {}. Expected 2-10", major));
        }
    }
...
fn validate_performance_expectations(
    ...
    if let Some(cpu) = &report.cpu {
        ...
        if cpu.cores >= 8 {
            let has_fast_storage =
                report.storage.iter().any(|s| s.device_type == "NVMe" || s.device_type == "SSD");
